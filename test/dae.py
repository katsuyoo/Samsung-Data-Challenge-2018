...
class DAE(object):
    ...
    def modeling(self,
        ...
        with self.graph.as_default():
            ...
            for n in range(len(self.encoder_layers) -1):
                w.append(tf.Variable(tf.truncated_normal([self.encoder_layers[n], self.encoder_layers[n+1]], mean = 0, stddev = 1.0 / np.sqrt(size_input_attribute))))
    ...
    def training(self, epochs=100):
        ...
        with tf.Session(graph= self.graph) as sess:
            self.saver.restore(sess, self.savepath_auxiliary)
            attributes = ["발생건수", "사상자수", "사망자수", "중상자수", "경상자수"]
            temp = self.X[attributes]          
            feedin = {self.X : temp}
            y = pd.get_dummies(pd.DataFrame(np.rint(sess.run(self.Y_hat, feed_dict = feedin)).astype(str))) # numpy list of 
            self.X.append(y)
        """
        with tf.Session(graph= self.graph) as sess:
            sess.run(self.init)
    ...
    def restoration(self,test_data):
        ...
        with tf.Session(graph= self.graph) as sess:
            ...
            self.Y = pd.DataFrame(sess.run(self.output, feed_dict= feedin), columns= self.origin_data.columns)
