...
class auxiliary_net:
    ...
    def __init__(self, session, input_size, output_size, data_type, savepath = 'tmp_auxiliary/auxiliary', no_levels = None, name = "auxiliary_net"):
    ...
    def build_network(self, h_size = 16, l_rate = 1e-3, p = 0.1, reg =0.05):
    ...
    def predict(self, x_test, y_test):
        test_loss = self.session.run(self.loss, feed_dict = {self.X: x_test, self.Y: y_test})          # type np.array of shape = [N, self.output_size]
        y_pred = self.session.run(self.Y_hat, feed_dict = {self.X : x_test})
